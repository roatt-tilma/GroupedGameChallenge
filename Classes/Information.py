...
class Question:
    ...
    def get_answer(self, frame):
        ...
        key_options = {i:self.options[i] for i in range(len(self.options))} #to provide key to each option
...
class Admin:    
    ...
    def __get_correct_answer(self, options, f, ADQ): 
        ...
        menu = OptionMenu(ADQ, var, *options) #creating options menu to choose the correct answer among the given options
    ...
    def __add_options(self, f, ADQ):
        ...
        def done(): #when admin has given all the options
            for widget in ADQ.winfo_children():
                widget.destroy()
            f.write("\n")
            self.__get_correct_answer(options, f, ADQ) #calling get_correct_answer method
    ...
    def __build_question_objects(self, li):
        ...
        for el in li:
            question_att = [i for i in el.splitlines() if i] #splitting the lines
            quest = question_att[0] #first line is the question
            options = question_att[1].split("||") #second line is options separated by ||
            options.pop() #removing the last ||
            right_answer = question_att[2] #third line is the correct answer
            diff_lvl, base_points = tuple(map(int, question_att[3].split())) #fourth line is difficulty level and base points separated by a space
